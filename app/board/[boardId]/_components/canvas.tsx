...
export const Canvas = ({ boardId }: CanvasProps) => {
	...
	const insertLayer = useMutation(
		(
			...
			if (liveLayers.size >= MAX_LAYERS) {
				return toast.error(`Can't place more layers. ${MAX_LAYERS} is a maximum amount of layers.`)
			}
	...
	const onWheel = useCallback((e: React.WheelEvent) => {
		setCamera(prevCamera => {
			const newCamera = {
				x: prevCamera.x - (e.shiftKey ? e.deltaY : e.deltaX),
				y: prevCamera.y - (e.shiftKey ? e.deltaX : e.deltaY),
			}
			return newCamera
		})
	}, [])
			...
			<svg
				...
				<g style={{ transform: `translate(${camera.x}px, ${camera.y}px)` }}>
					...
					{pencilDraft != null && pencilDraft.length > 0 && (
						<Path points={pencilDraft} x={0} y={0} fill={colorToCss(lastUsedColor)} />
					)}
				</g>
			</svg>
		</main>
